...
import org.example.icatch.dto.CameraSetupRequest; 
...
public class CameraController {
    ...
    public ResponseEntity<Map<String, Integer>> setupCamera(
            @RequestBody CameraSetupRequest request) {

        Integer cameraId = cameraService.setupCamera(
                request.getUserId(),
                request.getDeviceId(),
    ...
    @GetMapping("/{cameraId}/monitor")
    public ResponseEntity<Map<String, Object>> getMonitoringStream(
            @PathVariable Integer cameraId,
            @RequestParam Integer userId) {

        Map<String, Object> response = new HashMap<>();
        response.put("streamUrl", "rtsp://example.com/stream/" + cameraId);
